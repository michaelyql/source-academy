...
function evaluate(program) { 
    ...
    while (! is_null(C)) {
        ...
        if (is_literal(command)) {
            ...
            const functions = filter(s => head(s) === "function_declaration",
                                     body);
            const everything_else = filter(s => head(s) !== "function_declaration",
                                           body);
            const new_sequence = append(functions, everything_else);
            // display_list(new_sequence, "new sequence");
            
            
            C = is_null(new_sequence)
                ? pair(make_literal(undefined), C)
                : is_null(tail(new_sequence))
                ? pair(head(new_sequence), C)
                : pair(head(new_sequence),
                    pair(make_pop_instruction(),
                      pair(make_sequence(tail(new_sequence)),
                        C)));
